...
type WhyFileCountCardProps = {
  icon: ElementType;
  title: string;
  description: string;
};

export function WhyFileCountCard({
  icon: Icon,
  title,
  description,
}: WhyFileCountCardProps) {
  return (
    <article
      className={cn(
        "why-filecount-card group relative flex w-full max-w-[350px] flex-col justify-center overflow-hidden border bg-[var(--header-background)]",
      )}
      style={{
